import { ArrowDownIcon, ArrowUpIcon, ChatAltIcon } from '@heroicons/react/outline'
...
function Post({post}: Props) {
    ...
    const upVote = async (isUpvote: boolean) => {
        if(!session) {
            toast("⬆️ You need to sign in to Vote!")
            return
        }
        ...
        <div className='rounded-md flex cursor-pointer border border-gray-300 bg-white shadow-sm hover:border hover:border-gray-600'>
            ...
            <div className="p-3 pb-1">
                ...
                <div className='flex items-center space-x-2'>
                    <Avatar seed={post.subreddit[0]?.topic} style='identicon'/>
                    <p className='text-xs text-gray-400'>
                    <Link href={`/b/${post.subreddit[0]?.topic}`}>
                        <span className='font-bold text-black hover:text-blue-400'>
                            b/{post.subreddit[0]?.topic}
                        </span>
                    </Link>
